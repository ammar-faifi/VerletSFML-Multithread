...
Renderer::Renderer(PhysicSolver &solver_, tp::ThreadPool &tp)
    : solver{solver_}, world_va{sf::Quads, 4}, objects_va{sf::Quads},
      thread_pool{tp} {
  initializeWorldVA();
...
void Renderer::updateParticlesVA() {
  ...
  thread_pool.dispatch(to<uint32_t>(solver.objects.size()), [&](uint32_t start,
                                                                uint32_t end) {
...
void Renderer::renderHUD(RenderContext &) {
  ...
  text_objects.setString("Objects: " +
  toString(simulation.solver.objects.size())); text_objects.setPosition({margin,
  current_y}); current_y += text_objects.getBounds().y + 0.5f * margin;
