...
struct PhysicSolver {
  ...
  PhysicSolver(IVec2 size, tp::ThreadPool &tp)
      : grid{size.x, size.y}, world_size{to<float>(size.x), to<float>(size.y)},
  ...
  void solveCollisions() {
    ...
    for (uint32_t i{0}; i < thread_count; ++i) {
      thread_pool.addTask(
          [this, i, slice_size] { solveCollisionThreaded(2 * i, slice_size); });
    }
  ...
  // Add a new object to the solver
  uint64_t createObject(Vec2 pos) { return objects.emplace_back(pos); }
  ...
  void addObjectsToGrid() {
    ...
    for (const PhysicObject &obj : objects.data) {
      if (obj.position.x > 1.0f && obj.position.x < world_size.x - 1.0f &&
        ...
        grid.addAtom(to<int32_t>(obj.position.x), to<int32_t>(obj.position.y),
                     i);
      }
      ++i;
    }
  ...
  void updateObjects_multi(float dt) {
    thread_pool.dispatch(to<uint32_t>(objects.size()),
