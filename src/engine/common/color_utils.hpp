...
struct ColorUtils {
  template <typename T> static sf::Color createColor(T r, T g, T b) {
    return {to<uint8_t>(r), to<uint8_t>(g), to<uint8_t>(b)};
  }

  template <typename TVec3> static sf::Color createColor(TVec3 vec) {
    return {to<uint8_t>(vec.x), to<uint8_t>(vec.y), to<uint8_t>(vec.z)};
  }

  static sf::Color interpolate(sf::Color color_1, sf::Color color_2,
                               float ratio) {
